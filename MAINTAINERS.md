...
| `Source/Hosts/<Variant>/LowCodeApp.<Own>` | バリアント固有プロジェクトのマスタ（`Cookie` のサーバーとクライアント、`Maui`、`Wpf`、`WinForms`）。公開テンプレートは `Cookie`（接尾辞なし・既定）/ `Maui` / `Wpf` / `WinForms`。`MultiTenant` は `Variants.cs` で `IsTemplate: false`（ソリューション生成とデバッグ用コピーの対象だが VSIX には入れない） |
