...
namespace LowCodeApp.Server.Controllers
{
    ...
    public class ModuleDataController : ControllerBase, IAsyncDisposable
    {
        ...
        public async Task<IActionResult> GetDesignData()
        {
            ...
            await LicenseService.UpdateAsync(Request, _dataService.UserInfo);
            await _dataService.ModuleDataIO.CheckAppAuthorization();
            return this.FileWithETag(DesignerService.GetDesignDataForFront(_dataService.TenantKey, await _dataService.ModuleDataIO.GetCurrentUser()), "application/octet-stream");
        }
        ...
        {
            await _dataService.InitializeAsync();
            return Ok(await BulkFileTransfer.GetListFileAsync(DesignerService.GetDesignData(_dataService.TenantKey), _dataService.ModuleDataIO, condition!));
        }
        ...
        {
            await _dataService.InitializeAsync();
            return await BulkFileTransfer.SubmitByFileAsync(DesignerService.GetDesignData(_dataService.TenantKey), _dataService.ModuleDataIO, moduleName, Request.Body);
        }
        ...
        {
            await _dataService.InitializeAsync();
            return Ok(await BulkFileTransfer.GetListFileByDataAsync(DesignerService.GetDesignData(_dataService.TenantKey), _dataService.ModuleDataIO, moduleName, Request.Body));
        }
        ...
        public async Task<IActionResult> ParseFileAsync(string? moduleName)
        {
            ...
            return Content(Codeer.LowCode.Blazor.Json.JsonConverterEx.SerializeObject(
                await BulkFileTransfer.ParseFileAsync(DesignerService.GetDesignData(_dataService.TenantKey), _dataService.ModuleDataIO, moduleName, Request.Body)),
                "application/json");
        }

        [HttpGet("resource")]
        public async Task<IActionResult> GetResourceAsync(string? resource)
        {
            await _dataService.InitializeAsync();
            var mem = DesignerService.GetResource(_dataService.TenantKey, resource ?? string.Empty);
